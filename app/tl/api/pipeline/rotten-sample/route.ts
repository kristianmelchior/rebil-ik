// GET /tl/api/pipeline/rotten-sample?owner=<name>&limit=10
// Returns up to `limit` rotten deals for the given owner name.
...
export async function GET(request: Request) {
  ...
  const { data, error } = await getAdminClient()
    .rpc('get_rotten_sample', { p_owner_name: owner, p_limit: limit })
